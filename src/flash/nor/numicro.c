...
static const struct numicro_cpu_type NuMicroParts[] = {
	...
	/* M253 */
	{"M252GZE3AE", 0x00F250E0, NUMICRO_BANKS_GENERAL(128*1024, 0*1024, 4*1024, 12)},
	{"M252GLE3AE", 0x00F25000, NUMICRO_BANKS_GENERAL(128*1024, 0*1024, 4*1024, 12)},
...
static int numicro_erase(struct flash_bank *bank, int first, int last)
{
	...
	numicro_get_arm_arch(target);
	if (!bSPIMFlashWrite || m_M23SecureDebugState == 2) {
		retval = numicro_init_isp(target);
	}
	if (retval != ERROR_OK)
		return retval;
