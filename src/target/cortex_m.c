...
int cortex_m_examine(struct target *target)
{
	...
	if (!target_was_examined(target)) {
		...
		if (i == 4) {
			...
			armv7m->arm.is_NUC_M23_FMC_MSB5 = false;
			retval = target_read_u32(target, NUC_M23_FLASH_VERSION, &regValue);
			if (retval != ERROR_OK) {
				LOG_DEBUG("Could not read the value of NUC_M23_FLASH_VERSION");
			}
			else {
				if (regValue == NUC_M23_FLASH_MSB5) {
					LOG_DEBUG("MSB of FMC of the Nuvoton M23 chip is 5 (NUC_M23_FLASH_VERSION: 0x%" PRIx32 ").", regValue);
					armv7m->arm.is_NUC_M23_FMC_MSB5 = true;
				}
				else {
					LOG_DEBUG("MSB of FMC of the Nuvoton M23 chip is 4 (NUC_M23_FLASH_VERSION: 0x%" PRIx32 ").", regValue);
					armv7m->arm.is_NUC_M23_FMC_MSB5 = false;
				}
			}
